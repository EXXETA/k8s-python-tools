...
class DropNode(AbstractCommand):
    ...
    def run(self, args):
        ...
        if args.node:
            remove_node(context, args.node)
            wait_for_node_is_away(context, args.node)
        else:
            obj = confirm_node(context)
            if obj is not None and confirm("Do you really want to delete Node " + obj + "?"):
                remove_node(context, obj)
                wait_for_node_is_away(context, obj)
            else:
                print("No Node removal executed")
